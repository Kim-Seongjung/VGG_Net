...
def make_layer(layer_name ,weights_info, biases_info , device_name ,strides):
    """
    ...
    with tf.device(device_name):
        assert biases_info[1]== weights_info[4] ' # biases ch and # weights_output ch have to be same!' 
        with tf.variable_scope(layer_name) as scope:
            try: #처음 변수를 만드는 거면 그냥 만든다,
                tf.get_variable(weight_info[0], shape=[weight_info[1],weight_info[2] ,weight_info[3] , weight_info[4]])
                tf.get_variable(biases_info[0], shape= [biases_info[1]] )
            except:
                print " this weight and biases already existed! reuse those variable"
                scope.variable_reuse()
                tf.get_Variable(weight_name)


def make_weights(weight_size,n_in_ch ,n_out_ch ,GPU , reuse= True , strides = [1,1,1,1]):
    with tf.device('/gpu:'+GPU):
        with tf.variable_scope(layer_name) as scope:
            if w_conv1= tf.get_variable("")
